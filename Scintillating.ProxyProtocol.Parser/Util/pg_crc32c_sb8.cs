...
namespace Scintillating.ProxyProtocol.Parser.Util
{
    internal static class pg_crc32c_sb8
    {
        ...
        private static uint CRC8(ref uint ptr, uint crc, uint x)
        {
            if (!BitConverter.IsLittleEndian)
            {
                return Unsafe.Add(ref ptr, 0 * TABLE_SIZE + (((crc >> 24) ^ (x)) & 0xFF)) ^ (crc << 8);
            }

            return Unsafe.Add(ref ptr, 0 * TABLE_SIZE + ((crc ^ (x)) & 0xFF)) ^ (crc >> 8);
        }

        public static unsafe pg_crc32c pg_comp_crc32c_sb8(pg_crc32c crc, byte* data, nuint len)
        {
            ref uint ptr = ref Unsafe.As<byte, uint>(ref MemoryMarshal.GetArrayDataReference(pg_crc32c_table));
            ...
            while (len >= 8)
            {
                ...
                crc = Unsafe.Add(ref ptr, 0 * TABLE_SIZE + c0) ^ Unsafe.Add(ref ptr, 1 * TABLE_SIZE + c1) ^
                      Unsafe.Add(ref ptr, 2 * TABLE_SIZE + c2) ^ Unsafe.Add(ref ptr, 3 * TABLE_SIZE + c3) ^
                      Unsafe.Add(ref ptr, 4 * TABLE_SIZE + c4) ^ Unsafe.Add(ref ptr, 5 * TABLE_SIZE + c5) ^
                      Unsafe.Add(ref ptr, 6 * TABLE_SIZE + c6) ^ Unsafe.Add(ref ptr, 7 * TABLE_SIZE + c7);
        ...
        #region pg_crc32c_table

        private static readonly uint32[,] pg_crc32c_table = BitConverter.IsLittleEndian ? new uint32[8, TABLE_SIZE] {
        {
