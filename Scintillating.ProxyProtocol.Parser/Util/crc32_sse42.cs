...
internal static unsafe class crc32_sse42
{
    ...
    private static readonly uint* ptr_crc32c_long = crc32c_zeros(LONG, out crc32c_long);

    private static readonly uint[]? crc32c_2long;
    private static readonly uint* ptr_crc32c_2long = crc32c_zeros(2 * LONG, out crc32c_2long);

    private static readonly uint[]? crc32c_short;
    private static readonly uint* ptr_crc32c_short = crc32c_zeros(SHORT, out crc32c_short);
    ...
    private static uint sse42_crc32c_x86_64(uint crc, byte* buf, nuint len)
    {
        ...
        uint* _ptr_crc32c_long = ptr_crc32c_long;
        uint* _ptr_crc32c_2long = ptr_crc32c_2long;
        uint* _ptr_crc32c_short = ptr_crc32c_short;
        uint* _ptr_crc32c_2short = ptr_crc32c_2short;
        ...
        if (LONG > SHORT)
        {
            ...
            while (len >= LONG * 3)
            {
                ...
                crc = (uint)(crc32c_shift(_ptr_crc32c_long, crc) ^ crc0);
                crc1 = crc32c_shift(_ptr_crc32c_long, (uint)crc1);
                crc = (uint)(crc32c_shift(_ptr_crc32c_2long, crc) ^ crc1);
                crc0 = crc2;
                next += LONG * 2;
                len -= LONG * 3;
            }
        ...
        while (len >= SHORT * 3)
        {
            ...
            crc = (uint)(crc32c_shift(_ptr_crc32c_short, crc) ^ crc0);
            crc1 = crc32c_shift(_ptr_crc32c_short, (uint)crc1);
            crc = (uint)(crc32c_shift(_ptr_crc32c_2short, crc) ^ crc1);
            crc0 = crc2;
            next += SHORT * 2;
            len -= SHORT * 3;
        }
    ...
    private static unsafe uint sse42_crc32c_x86(uint crc, byte* buf, nuint len)
    {
        ...
        if (LONG > SHORT)
        {
            ...
            while (len >= LONG * 3)
            {
                ...
                crc = crc32c_shift(_ptr_crc32c_long, crc) ^ crc0;
                crc1 = crc32c_shift(_ptr_crc32c_long, crc1);
                crc = crc32c_shift(_ptr_crc32c_2long, crc) ^ crc1;
                crc0 = crc2;
                next += LONG * 2;
                len -= LONG * 3;
            }
        ...
        while (len >= SHORT * 3)
        {
            ...
            crc = crc32c_shift(_ptr_crc32c_short, crc) ^ crc0;
            crc1 = crc32c_shift(_ptr_crc32c_short, crc1);
            crc = crc32c_shift(_ptr_crc32c_2short, crc) ^ crc1;
            crc0 = crc2;
            next += SHORT * 2;
            len -= SHORT * 3;
        }
